...
function StatCard({ icon: Icon, label, value, suffix }: typeof stats[number]) {
  ...
  return (
    <div
      ...
      className="border border-navy-foreground/10 bg-navy-foreground/[0.03] p-5 border-l-4 border-l-saffron flex items-center gap-4"
    >
      <div className="size-11 rounded bg-navy-foreground/[0.06] grid place-items-center shrink-0">
        <Icon className="size-5 text-saffron" strokeWidth={1.5} aria-hidden />
      </div>
      <div className="min-w-0">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-navy-foreground opacity-60">{label}</p>
        <p className="mt-1 text-2xl md:text-3xl font-bold text-navy-foreground tabular-nums leading-none">
          {v.toLocaleString("en-IN")}
          {suffix && <span className="text-xs font-semibold text-navy-foreground opacity-60 ml-1">{suffix}</span>}
        </p>
...
function BarChart() {
  ...
  return (
    <div className="border border-navy-foreground/10 bg-navy-foreground/[0.03] p-5 h-full flex flex-col">
      <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-saffron">
        Drug-wise Arrest Statistics
      </p>
      <p className="text-xs text-navy-foreground opacity-60 mt-1">Arrests categorised by primary substance type</p>
      <div className="mt-5 flex gap-3 flex-1 min-h-[12rem]">
        <div className="flex flex-col justify-between text-[10px] text-navy-foreground opacity-50 text-right pr-1">
          {ticks.map((t) => (
            <span key={t}>{t >= 1000 ? `${(t / 1000).toString().replace(/\.0$/, "")}k` : t}</span>
          ))}
        </div>
        <div className="flex-1 relative border-l border-b border-navy-foreground/10">
          {ticks.slice(0, -1).map((_, i) => (
            <div
              key={i}
              className="absolute left-0 right-0 border-t border-navy-foreground/5"
      ...
      <div className="flex gap-2 pl-7 mt-2 text-[10px] text-navy-foreground opacity-60">
...
function Donut() {
  ...
  return (
    <div className="border border-navy-foreground/10 bg-navy-foreground/[0.03] p-5 h-full flex flex-col">
      ...
      <p className="text-xs text-navy-foreground opacity-60 mt-1">Distribution by role in trafficking ecosystem</p>
      ...
      <ul className="mt-4 grid grid-cols-2 gap-2 text-xs text-navy-foreground opacity-80">
        {arrestSegments.map((s) => (
    ...
    <section aria-labelledby="enf-title" className="bg-navy-deep text-navy-foreground py-16">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div>
          ...
          <p className="text-sm text-navy-foreground opacity-70 mt-3 max-w-2xl">
        ...
        {/* Wanted Fugitives */}
        <div className="mt-16 pt-12 border-t border-navy-foreground/10">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-saffron">
            ...
            {wanted.map((w) => (
              <li key={w.name} className="group flex flex-col">
                ...
                <div className="pt-3 text-center">
                  <p className="text-sm font-bold text-navy-foreground leading-tight">{w.name}</p>
